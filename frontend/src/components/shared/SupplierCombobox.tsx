...
export function SupplierCombobox({
  ...
  const { data: results, isFetching } = useQuery({
    ...
    queryFn: () => listSuppliers({ search: debouncedQuery || undefined, status: "active" }),
    enabled: open,
  });
  ...
  return (
    <div className="relative" ref={containerRef}>
      <Input
        placeholder="Buscar fornecedor pelo nome ou documento..."
        value={query}
          ...
          {isFetching ? (
            ...
            results.map((supplier) => (
              ...
              >
                ...
                {(supplier.trade_name || supplier.document) && (
                  <span className="text-xs text-muted-foreground">
                    {[supplier.trade_name, supplier.document].filter(Boolean).join(" · ")}
                  </span>
                )}
              </button>
            ))
