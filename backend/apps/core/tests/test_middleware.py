...
@override_settings(CORS_ALLOWED_ORIGINS=["http://localhost:5173"], FRONTEND_URL="http://localhost:5173")
...
def test_ignores_safe_methods_and_non_api_paths():
    mw = _mw()
    assert mw(rf.get("/api/customers/", HTTP_ORIGIN="http://evil.example")).status_code == 200
    assert mw(rf.post("/admin/login/", HTTP_ORIGIN="http://evil.example")).status_code == 200
