...
def resync(apps, schema_editor):
    ...
    Aqui: adiciona o módulo ``notifications`` (view/edit/test) para a gestão dos
    templates de notificação ao cliente. ``notifications.view`` é não crítica (o
    Administrador a recebe); edit/test são críticas (superuser ou concessão).
    """
...
class Migration(migrations.Migration):
    dependencies = [("accounts", "0005_resync_rbac_expense")]
